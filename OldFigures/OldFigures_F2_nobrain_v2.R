...
cog_ref_nb %>% group_by(year) %>% summarise(SA1 = sum(SA1, na.rm = T),
                                         ...
                                         SA4 = sum(SA4, na.rm = T),
                                         SA5 = sum(SA5, na.rm = T),
                                         total = n()) -> cog_ref_summ_nb

cog_ref_summ_nb$total_SA_hits <- cog_ref_summ_nb$SA1 + cog_ref_summ_nb$SA2 + cog_ref_summ_nb$SA3 + cog_ref_summ_nb$SA4 + cog_ref_summ_nb$SA5


aff_cit_plot <- ggplot(cit_links_summ, aes(x=year_cited)) +
  labs(y='Citations, Y') +
  geom_line(aes(y=SA2_F/total_hits_SA, color='SA1'),size=1) +
  geom_line(aes(y=SA1_ABG/total_hits_SA, color='SA2'),size=1) +
  geom_line(aes(y=SA5_HIKM/total_hits_SA, color='SA3'),size=1) + 
  geom_line(aes(y=SA3_CN/total_hits_SA, color='SA4'),size=1) +
  geom_line(aes(y=SA4_DEJL/total_hits_SA, color='SA5'),size=1) +
  xlim(1967, 2019) +
  ylim(0, 0.55) + 
  theme_bw() +
  ...
  scale_color_manual(name = "",       values = c("SA1" = "red",
                                                 "SA2" = "orange", 
                                                 "SA3"  = "yellow2", 
                                                 "SA4" = "green", 
                                                 "SA5" = "blue")) + 
...
ref_plot <- ggplot(cog_ref_summ, aes(x=year)) +
  labs(y='References') +
  geom_line(aes(y=SA2/total_SA_hits, color='SA1'),size=1) +
  geom_line(aes(y=SA1/total_SA_hits, color='SA2'),size=1) +
  geom_line(aes(y=SA5/total_SA_hits, color='SA3'),size=1) +
  geom_line(aes(y=SA3/total_SA_hits, color='SA4'),size=1) +
  geom_line(aes(y=SA4/total_SA_hits, color='SA5'),size=1) +
  ggtitle("Cognitive") +
  xlim(1967, 2019) +
  ylim(0, 0.55) + 
  theme_bw() +
...
ref_plot_nb <- ggplot(cog_ref_summ_nb, aes(x=year)) +
  labs(y='References') +
  geom_line(aes(y=SA2/total_SA_hits, color='SA1'),size=1) +
  geom_line(aes(y=SA1/total_SA_hits, color='SA2'),size=1) +
  geom_line(aes(y=SA5/total_SA_hits, color='SA3'),size=1) +
...
cit_plot2_nb <- ggplot(cog_cit_summ2_nb, aes(x=yearP)) +
  labs(y='Citations, P') +
  geom_line(aes(y=SA2/total_SA_hits, color='SA1'),size=1) +
  geom_line(aes(y=SA1/total_SA_hits, color='SA2'),size=1) +
  geom_line(aes(y=SA5/total_SA_hits, color='SA3'),size=1) +
...
png('C:/Users/Rubinzone/Desktop/F2_hires.png', width = 12, height = 8, units = 'in', res = 300)

ggarrange(aff_ref_plot + rremove("x.text") , 
          ref_plot_nb + rremove("x.text") + rremove("y.text"),
          aff_cit_plot_publ, 
          cit_plot2_nb + rremove("y.text"),
          ncol = 2, nrow = 2,
          heights = c(1.2,1.2,1,1),
          widths = c(1.15,1,1.15,1),
