...
sankey <- function(var_cit_links, var_ref_links, png_file_prefix){
  ...
  var_cit_links[var_cit_links$SA1_ABGp >= 1,] %>% group_by(year_cited) %>% summarise(
  ...
  var_cit_links[var_cit_links$SA2_Fp >= 1,] %>% group_by(year_cited) %>% summarise(
    SA1_ABG = sum(SA1_ABG, na.rm = T),
    SA2_F = sum(SA2_F, na.rm = T),
    SA3_CN = sum(SA3_CN, na.rm = T),
    SA4_DEJL = sum(SA4_DEJL, na.rm = T),
  ...
  var_cit_links[var_cit_links$SA3_CNp >= 1,] %>% group_by(year_cited) %>% summarise(
    SA1_ABG = sum(SA1_ABG, na.rm = T),
    SA2_F = sum(SA2_F, na.rm = T),
    SA3_CN = sum(SA3_CN, na.rm = T),
    SA4_DEJL = sum(SA4_DEJL, na.rm = T),
  ...
  var_cit_links[var_cit_links$SA4_DEJLp >= 1,] %>% group_by(year_cited) %>% summarise(
    SA1_ABG = sum(SA1_ABG, na.rm = T),
    SA2_F = sum(SA2_F, na.rm = T),
    SA3_CN = sum(SA3_CN, na.rm = T),
    SA4_DEJL = sum(SA4_DEJL, na.rm = T),
  ...
  var_cit_links[var_cit_links$SA5_HIKMp >= 1,] %>% group_by(year_cited) %>% summarise(
    SA1_ABG = sum(SA1_ABG, na.rm = T),
    SA2_F = sum(SA2_F, na.rm = T),
    SA3_CN = sum(SA3_CN, na.rm = T),
    SA4_DEJL = sum(SA4_DEJL, na.rm = T),
...
sankey(subset(cit_links, year_cited <= 1965), 
       subset(ref_links, year_references <= 1965),
       "1950_1965")
sankey(subset(cit_links, year_cited > 1965 & year_cited <= 1990),
       subset(ref_links, year_references > 1965 & year_references <= 1990),
       "1966_1990")
sankey(subset(cit_links, year_cited >= 1991 & year_cited <= 2000),
       subset(ref_links, year_references >= 1991 & year_references <= 2000),
       "1991_2000")
sankey(subset(cit_links, year_cited >= 2001 & year_cited <= 2010),
       subset(ref_links, year_references >= 2001 & year_references <= 2010), 
       "2001_2010")
sankey(subset(cit_links, year_cited >= 2011 & year_cited <= 2020),
