...
cL1_plot_nb <- ggplot(data.frame("L1" = rowSums(cog_subj_bool_nb[,c(2:17)])), aes(x=L1)) + 
  geom_bar(color="black", fill="black") + 
  scale_x_continuous(name="Medical Subject Headings (MeSH)",breaks=c(1:20), limits=c(0,10)) +
  scale_y_continuous(name=as.expression(bquote("Cognitive , no brain key (x" ~ 10^3 ~ ")")), labels = label_number(suffix = " ", accuracy = 1, scale = 1e-3), limits = c(0,280000)) +
  theme_void() +
  theme(axis.text.x=element_text(size=rel(1.3)),
        axis.text.y=element_text(size=rel(1.3)),
        axis.title.y=element_text(size=rel(1.5),angle=90,family="NimbusMon", face="bold", margin = margin(t = 0, r = 10, b = 0, l = 0)),
...
png('C:/Users/Rubinzone/Desktop/F3_hires.png', width = 8, height = 12, units = 'in', res = 300)

ggarrange(aL1_plot + rremove("x.text"),
          aSA_plot + rremove("y.text") + rremove("x.text"),
          cL1_plot + rremove("x.text"),
          cSA_plot + rremove("y.text") + rremove("x.text"),
          cL1_plot_nb,
          cSA_plot_nb,
          ncol = 2, nrow = 3,
          heights = c(1,1,1,1,1.2,1.2),
          widths = c(1.2,1,1.2,1,1.2,1))
